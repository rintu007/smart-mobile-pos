...
| --- | --- | --- | --- | --- |
| Auth (`/auth/*` sign-in, OTP request) | Per account, and separately per IP | 5 sign-in attempts/minute per account; 20 OTP requests/hour per account; 30 requests/minute per IP | Brute-force and OTP-spam resistance — sized against attack patterns, not legitimate use, which never approaches this. | **Server-side: cannot be implemented in this codebase.** Sign-in is a direct client call to Supabase Auth (`docs/modules/authentication/specification.md` §1a) — it never reaches an `apps/web` Route Handler, so there is no request here for `requirePermission`'s counter to attach to. This needs Supabase Auth's own platform-side rate limiting (Dashboard → Authentication → Rate Limits — a real, named Supabase feature, not a hypothetical one); whether it's already active by default has not been separately confirmed, and confirming that is itself a founder action, not something already done. **Client-side, built Sprint 64:** re-examined rather than left at "purely founder-blocked" once Sprints 62/63 found real engineering work hiding inside two other items with the same framing — found the mobile client did nothing at all to slow repeated failed attempts. `SignInController` now free-passes the first 3 failures, then applies an exponential cooldown (5s, 10s, 20s, 40s, capped at 60s) before the *same app instance* can retry, resetting on any success. Defense-in-depth only — it throttles one device's own retry loop, not an account across devices, and a full app restart resets it; the server-side gap above is unchanged and still the real control. |
...
| 0.3.0 | 2026-08-21 | Sprint 64 — re-examined the Auth row's "cannot be implemented" framing, the same scrutiny Sprints 62/63 applied to two other "purely founder-blocked" items that turned out to have real engineering work hiding inside them. The server-side claim holds fully (sign-in genuinely never reaches an `apps/web` Route Handler); but nothing on the client throttled repeated failed attempts either, which is ordinary client-side defense-in-depth, independent of any Supabase setting. Built: `SignInController` free-passes 3 failures, then applies an exponential cooldown (5s–60s, capped) before the same app instance can retry. Named honestly as a narrower guarantee than server-side rate limiting (one device's own retry loop, not an account; resets on app restart). |
