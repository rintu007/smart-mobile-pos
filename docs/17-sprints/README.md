...
> **Status:** 🔵 In review — all 5 deliverables drafted; Sprint 01 through Sprint 08 all closed
> **Version:** 0.10.0
...
| [`backlog.md`](backlog.md) | M0 fully decomposed into 13 estimated, dependency-ordered items (21 person-days, items 12/13 added during Sprint 06/08 planning) | 🔵 In review |
...
| [`sprint-08.md`](sprint-08.md) | Store context — `GET /api/v1/stores` + mobile fetch-and-cache — verified live with a cross-tenant RLS proof | 🟢 Done |
...
| 0.10.0 | 2026-08-02 | Sprint 08 planned and closed same-day: found a real gap while planning the till screen (no `store_id` source existed for the device) and scoped a dedicated sprint to it — `GET /api/v1/stores` built and verified live with a cross-tenant RLS proof, mobile fetch-and-cache built. Fixed a stale doc claim (RLS on `stores` was live since Sprint 02, spec still said otherwise). Backlog total now past roadmap.md's estimate, reported honestly. |
