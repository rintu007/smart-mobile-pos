...
> **Status:** Done — both backlog items complete. Identity/Auth: live Supabase project, schema migration, Custom Access Token Hook (Dashboard-wired), RLS on `tenants`/`users`, demo script run end-to-end and passed. Repository/CI: branch protection live on `main`, `pr.yml` actually exercised for the first time via [PR #1](https://github.com/rintu007/smart-mobile-pos/pull/1) (which itself found and fixed two real gaps: a missing `packageManager` pin and an eslint rule flagging Next's own autogenerated `next-env.d.ts`), merged. See [implementation-log.md](../18-implementation/implementation-log.md) for full detail.
...
- [x] Tests pass **locally** on a clean `pnpm install` (lint, typecheck, 2 unit tests, build all
      verified) — [x] **in CI**: [PR #1](https://github.com/rintu007/smart-mobile-pos/pull/1) ran
      `pr.yml` for the first time and, after two real fixes (missing `packageManager` pin, an
      eslint rule flagging `next-env.d.ts`), all three jobs passed and the PR merged
...
1. From a clean checkout, run the documented setup sequence ([repository-setup.md §4](../15-github-project/repository-setup.md#4-reproducibility-from-a-clean-checkout)).
   **Run 2026-08-01, passed** — `pnpm install` from a fresh clone, verified via the CI runner itself.
2. Open a pull request with no code changes (a scaffold commit) — show branch protection blocking a
   direct push to `main` and CI running and passing. **Run 2026-08-01, passed** —
   [PR #1](https://github.com/rintu007/smart-mobile-pos/pull/1): direct push to `main` was blocked
   by Claude Code's own safety layer before even reaching GitHub (the right outcome, tested via the
   proper branch+PR flow instead); `pr.yml` ran for real for the first time, found two genuine gaps
   (see Definition of Done above), both fixed, then passed and merged.
...
| 0.4.0 | 2026-08-01 | `gh` CLI installed and authenticated (founder completed device-code login); repo visibility resolved as public (branch protection requires it on GitHub's free tier — a real cost/privacy tradeoff, decided by the founder); branch protection configured on `main` (required status checks, admin-included, 0 required approvals per the solo-founder compensating control in repository-setup.md §3); PR #1 opened, found and fixed two real CI gaps, passed all checks, and merged. Sprint 01 is fully done — both backlog items complete and demoed on real infrastructure. |
