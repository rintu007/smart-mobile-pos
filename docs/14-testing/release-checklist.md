...
| ☐ Server-testable and mobile-testable failure scenarios passing (**corrected from "all 10 offline failure scenarios passing"** — test-plan.md §3's Sprint 41 finding: only 1 of the 10 named scenarios in failure-scenarios.md §1 is a server-side, automatable case; **updated Sprints 50–52** — 4 more are mobile-testable with existing `flutter test` infrastructure, found once actually attempted; the remaining 5 need the full Supabase CLI stack or are already resolved as "not applicable"/"not a failure"/"proven by design" with nothing further to test) | [failure-scenarios.md](../13-offline-sync/failure-scenarios.md) | ⬜ **Still not satisfied, but narrower — a real, material pilot risk, not a documentation nicety.** 5 of 10 scenarios now have real automated verification: the 1 server-testable case (`fast-integration`, every PR), "App killed mid-sync"/"Device rebooted with a full queue" (Sprint 50), "Schema version mismatch" (Sprint 51 — which found and fixed a real production bug in the process, see schema-local.md), and the client half of "Connectivity lost mid-batch" (Sprint 52 — which found the client-side guarantee needed no live server/proxy after all, and a third instance of the same doc-vs-code gap the two sprints before it found). Of the remaining 5: **2 are resolved with nothing to test** (already-not-a-failure/not-applicable, per failure-scenarios.md §1 itself), **1 is proven correct by design rather than by test** (a clock skewed by +36 hours, per clock-and-ordering.md §4), and **2 genuinely have zero verification on record** — a token expired mid-queue, and storage full. A real pilot shop's Cashier *will* eventually have their phone die mid-shift, reboot with a full queue, update across a schema-migrating release, or lose connectivity mid-sale — the four scenarios most likely to actually occur are now the four most recently closed; the remaining unverified ones are individually less likely but still real, and Phase 13's own charter still exists to catch exactly this class of failure. |
...
rows (down from four since Sprint 44): the failure-scenarios row (3 of 10 named scenarios still
genuinely unverified, down from 9 as of Sprint 52), the OWASP review's two remaining findings (RLS,
