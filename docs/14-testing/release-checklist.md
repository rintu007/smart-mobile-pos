...
the others; it is **not yet assessable**, since `backlog.md §6` item 2 (real founder-personal outreach
to candidate shops) hasn't happened yet — this row becomes live when that specific action starts,
not merely when M5 itself does (M5 has already begun, Sprint 65 — corrected phrasing). Of the remaining two rows, the Android-sideload row and the
...
| 0.15.1 | 2026-08-26 | Full re-check against Sprints 65–74 (M5 decomposition, Dependabot triage, two rounds of module-registry staleness audits): every substantive row confirmed unchanged and still accurate — none of that work touched RLS, sign-in rate limiting, Android signing, or MTS status. The one real drift found: the pilot-consent row and bottom-line paragraph both said "M5 hasn't started," imprecise since Sprint 65 actually decomposed M5 into item grain — corrected to name the real trigger condition (`backlog.md §6` item 2, founder-personal shop outreach, not yet done) rather than the milestone as a whole (already begun). No other correction needed — this checklist held steady through ten sprints of unrelated work, a reassuring result rather than a gap. |
