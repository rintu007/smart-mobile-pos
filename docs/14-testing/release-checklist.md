...
| ☐ A properly-signed (non-debug) release APK exists and can be sideloaded directly onto a pilot shop's device (**narrowed from "Google Play Console's Internal Testing track," Sprint 61** — see `cd-workflows.md §2`'s own correction: Play Console is this project's designed mechanism for scaling *past* founder-personal visits, not a requirement for the specific 2–3-shop, founder-visited first pilot `pilot-plan.md` actually describes) | [cd-workflows.md §2](../15-github-project/cd-workflows.md#2-android-build-signing-and-distribution), [pilot-plan.md](../16-milestones/pilot-plan.md) | ⬜ **Narrowed, Sprint 61 (previously a heavier "New row, Sprint 58").** What's actually still missing is smaller than Sprint 58 first framed it: a real, founder-provisioned signing keystore, so `flutter build apk --release` stops using the debug keystore (`apps/mobile/android/app/build.gradle.kts`'s own TODO) — then direct sideload during the founder's own day-one visit, the identical install mechanism already proven across Sprints 10/16/48/54's real-device installs. No CI pipeline, GitHub secret, or Play Console access is actually required for *this* box. Still not satisfied — this session cannot generate a real production keystore — but the founder's own remaining task here is now correctly scoped as "provision one keystore," not "stand up an entire release pipeline." |
