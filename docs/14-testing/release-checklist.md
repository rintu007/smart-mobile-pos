...
| ☐ Server-testable and mobile-testable failure scenarios passing (**corrected from "all 10 offline failure scenarios passing"** — test-plan.md §3's Sprint 41 finding: only 1 of the 10 named scenarios in failure-scenarios.md §1 is a server-side, automatable case; **updated Sprint 50** — 2 more are mobile-testable with existing `flutter test` infrastructure, found once actually attempted; the remaining 7 need the full Supabase CLI stack, a mobile `integration_test` + fault-injecting proxy, or are already resolved as "not applicable"/"not a failure" with nothing to test) | [failure-scenarios.md](../13-offline-sync/failure-scenarios.md) | ⬜ **Still not satisfied, but narrower — a real, material pilot risk, not a documentation nicety.** 3 of 10 scenarios now have real automated verification: the 1 server-testable case (`fast-integration`, every PR) plus "App killed mid-sync"/"Device rebooted with a full queue," built Sprint 50 (`sync_repository_test.dart`). Of the remaining 7: **2 are resolved with nothing to test** (already-not-a-failure/not-applicable, per failure-scenarios.md §1 itself) and **5 genuinely have zero verification on record** — a clock skewed by +36 hours (proven correct by design, not by a test, per clock-and-ordering.md §4), a token expired mid-queue, the client half of connectivity lost mid-batch, schema version mismatch, and storage full. A real pilot shop's Cashier *will* eventually have their phone die mid-shift or reboot with a full queue — the two scenarios most likely to actually occur are now the two most recently closed; the remaining unverified ones are individually less likely but still real, and Phase 13's own charter still exists to catch exactly this class of failure. |
...
rows (down from four since Sprint 44): the failure-scenarios row (5 of 10 named scenarios still
genuinely unverified, down from 9 as of Sprint 50), the OWASP review's two remaining findings (RLS,
