...
| 0.52.0 | 2026-08-19 | Sprint 46 (cross-cutting fix, not a numbered M4 item): customer-erasure anonymisation built — the third of Sprint 43's flagged findings closed this way, and like rate limiting (Sprint 45), one safe to close immediately with no production-configuration risk. `privacy.md §4`'s anonymise-not-delete resolution (null `name`/`phone`, keep the row/id so historical `sales.customer_id` FKs stay valid) had been fully designed since Phase 12 but never implemented. Built `POST /customers/{id}/erase` (Owner only — one level stricter than `DELETE`'s Manager+Owner gate, since this is a data-governance/legal action). A new explicit `erased_at` column, following this schema's own established convention of explicit state markers over inferred state. Also sets `deactivated_at` if not already set, without ever overwriting a genuinely earlier deactivation timestamp — caught and fixed during implementation, before any test was written, when an initial draft would have overwritten it unconditionally; a dedicated unit test and integration test now both assert the preservation directly. Found a related, previously-unexposed gap in the same pass: `deactivated_at` had never been surfaced in any customer API response at all, despite `DELETE /customers/{id}` setting it since Sprint 31 — fixed alongside the new `erased_at` field. Mobile UI explicitly out of scope — no admin surface exists yet for an Owner to initiate an erasure request. `privacy.md`, `customers.md`, `customers/specification.md`, `permission-matrix.md`, `owasp-checklist.md` all updated. 94/94 integration checks (90 pre-existing + 4 new), 215/215 unit tests (211 pre-existing + 4 new), `tsc`/`eslint` clean. |
| 0.53.0 | 2026-08-19 | Sprint 47 (cross-cutting fix, not a numbered M4 item): mobile secure token storage built — the fourth of Sprint 43's flagged findings closed this way, and the first mobile-side one. `flutter_secure_storage` was a `pubspec.yaml` dependency but had never actually been imported anywhere in `apps/mobile/lib` — `Supabase.initialize()` passed no custom `localStorage`, so the session sat in `supabase_flutter`'s own default, plaintext-on-Android `SharedPreferences`. Built `SecureLocalStorage` (`core/auth/secure_local_storage.dart`), implementing `supabase_flutter`'s `LocalStorage` behind a narrow, testable `SecureKeyValueStore` seam — production wires the real `flutter_secure_storage` package behind a thin adapter, tests use a genuine in-memory fake rather than mocking a platform channel, a first for this test suite. No migration from the old plaintext-stored sessions — a dated decision, since no real installed base exists yet (`release-checklist.md`'s "not pilot-ready today" status, Sprint 44). Found and fixed a real, live issue via `flutter analyze`: an `AndroidOptions(encryptedSharedPreferences: true)` parameter is deprecated in the installed `flutter_secure_storage` version (10.x auto-migrates to its own ciphers by default) — removed. `data-protection.md` corrected: its original §3 citation for the OWASP checklist's M1 claim conflated the SQLCipher database key (still unbuilt, M9) with the session token (now built) — split into a new §3a specifically for the latter. `owasp-checklist.md`'s M1 and M10 rows updated. `flutter analyze` clean, `flutter test` 244/244 (239 pre-existing + 5 new). |
| 0.54.0 | 2026-08-19 | Sprint 48 (cross-cutting fix, not a numbered M4 item): on-device database encryption built — the fifth of Sprint 43's flagged findings closed, and the last one that was genuinely unblocked engineering rather than founder-blocked (RLS, Android signing) or intentionally out-of-phase (A09 alerting, per `incident-response.md`'s own Phase 18 deferral). `data-protection.md §3` had decided on SQLCipher since Phase 12 but deliberately deferred the exact integration package to actual implementation time; that deferral turned out to matter — the anticipated `sqlcipher_flutter_libs` plugin is a no-op stub as of version 0.7.0 (confirmed by reading its own bundled README), because `package:sqlite3` 3.x now resolves its native library through Dart's own hooks/native-assets build system. The real integration is a `pubspec.yaml` declaration (`hooks.user_defines.sqlite3.source: sqlcipher`), not a plugin dependency. Built `getOrCreateDatabaseEncryptionKey` (a 256-bit random key generated once, stored via the same `SecureKeyValueStore` seam Sprint 47 built, made public for this second caller) and `AppDatabase.encrypted`, which opens the database via `drift_flutter`'s `DriftNativeOptions.setup` callback running SQLCipher's raw-key `PRAGMA key = "x'...'"` form (not a passphrase — the input is already 256 bits of randomness, so PBKDF2 stretching adds cost without adding security). Unlike Sprints 46/47, this touches real local data (unsynced sales, catalogue, an outbound queue), not trivially-regenerated session state, so a pre-existing plaintext database file is detected and reset once on first launch after this update (`legacy_database_reset.dart`) rather than either risking corruption by keying a plaintext file or building bespoke SQLCipher plaintext-export migration machinery — a real, separate scope disproportionate to this pre-pilot, no-real-installed-base product. `appDatabaseProvider` now throws if not overridden rather than ever silently opening an unencrypted database; `main.dart` resolves the key and overrides it at startup, the same override pattern its own tests already used for `storeContextProvider`/`autoSyncOnStartProvider`. Verified beyond `flutter analyze`/`flutter test`: a real `flutter build apk --debug --target-platform android-arm64` was produced and inspected directly (`libsqlcipher.so` confirmed present in the APK, not silently falling back to plain SQLite), and a dedicated test proves data written through a keyed connection is genuinely unreadable by a connection that never supplies the key, not just that `PRAGMA key` runs without error. `data-protection.md` §3 rewritten from "not yet built" to built; `owasp-checklist.md`'s M9 (fixed) and M10 (now fully CONFIRMED) rows updated. `flutter analyze` clean, `flutter test` 252/252 (244 pre-existing + 8 new). Of Sprint 43's six original findings, only RLS and Android signing remain — both founder-blocked, not engineering work this session can complete. |
| 0.55.0 | 2026-08-19 | Sprint 49 (cross-cutting closeout, not a numbered M4 item, same shape as Sprint 44): `release-checklist.md §2` — M4's own actual exit criterion — re-checked against Sprints 45–48's real results for the first time since Sprint 44's own pass. One row genuinely flips to satisfied: `nightly.yml` has now really fired on its own `schedule` trigger and passed, confirmed via `gh run list --workflow=nightly.yml`, not assumed from the workflow file's continued existence. One row's wording was corrected rather than left stale: the OWASP review's rate-limiting finding said "entirely unimplemented," which stopped being true the moment Sprint 45 shipped mutating/read/sync-push rate limiting — narrowed to what's actually still open, sign-in specifically (architecturally unreachable from this codebase, needs a Supabase-side platform check). RLS and the 9 unverified failure scenarios were re-confirmed unchanged, not silently assumed — checked `supabase/sql/*.sql` and `apps/web/src` directly for any new RLS-forcing or JWT-claim-setting code (none found) and the sprint docs/implementation log for any intervening failure-scenario coverage (none found). Honest bottom line, unchanged in direction: still not pilot-ready today, now three unresolved rows (9 unverified failure scenarios, RLS + sign-in rate limiting, MTS execution) instead of four. No code changes — documentation-accuracy work only, in both directions (this is the first correction pass on this checklist that moved a row toward satisfied, not only toward more honestly unsatisfied). |
| 0.56.0 | 2026-08-19 | Sprint 50 (cross-cutting fix, not a numbered M4 item, not a re-opening of item 6): 2 of the 9 unverified offline failure scenarios named in `test-plan.md §3` — "App killed mid-sync," "Device rebooted with a full queue" — built with existing `flutter test`/Drift infrastructure, no new tooling needed once actually attempted rather than assumed to need the same infra as the genuinely-deferred rows. Found and corrected a real doc/code gap while writing the test, before it could assert something that never happens: `SyncRepository` never writes the `Syncing` transitional status `state-machines.md`'s own Sync Item diagram specifies (a `Queued -> Syncing` transition on "attempt begins") — real code selects `queued`/`failed_retrying` rows, sends them, and only writes a new status once a real per-operation server result is known, so an interrupted push simply leaves the row untouched rather than stuck in a distinct in-flight state. The safety property still holds, via this simpler real mechanism (natural re-selection next cycle, server-side id-keyed upsert making the resend safe) rather than a client-side stale-row-detection step. Corrected across `state-machines.md`, `failure-scenarios.md §1`, `test-plan.md §3`, and the `outbound_queue` table's own docstring (which had claimed all 5 documented status values were live in practice; only 4 actually are). Narrows, but does not flip, `release-checklist.md`'s failure-scenarios row: 5 of 10 scenarios remain genuinely unverified, down from 9; the two closed here were judged the two most likely to actually occur for a real pilot shop's Cashier. `flutter analyze` clean, `flutter test` 254/254 (252 pre-existing + 2 new). |
| 0.57.0 | 2026-08-20 | Sprint 51 (cross-cutting fix, not a numbered M4 item): "Schema version mismatch after an update," the last of test-plan.md §3's mobile-only failure scenarios, built with the same `flutter test`/Drift infrastructure (`migration_test.dart`, this project's first migration test) — and found a real, previously-undetected production bug in the process, arguably the most severe finding this project's testing has produced to date. `Drift`'s `Migrator.createTable` builds a table from its *current* Dart definition, not the shape it had when that call was originally written — `shop_settings_cache` was created in one `onUpgrade` step (Sprint 37) and altered in a later one (Sprint 39, adding `footer_message`), so any real device jumping straight from before schema v8 to v9 in one update (this project's own APK-rebuild cadence doesn't align with every schema-changing sprint, making this a real, not theoretical, path) hit an unhandled duplicate-column `SqliteException` on every launch, permanently losing access to its own local database including any unsynced sales. Reproduced live before fixing (reverting the guard reproduces the exact crash), then fixed with a guarded `addColumn` that checks `pragma_table_info` for the column's existence first — verified both for the previously-broken jump (v3→v9, also proving the pre-existing `sales.created_at` backfill logic works, itself untested until now) and the already-working path (genuine v8→v9, proving the guard doesn't skip a real, needed column add). The standing lesson — any table created in one migration step and altered in a later one needs this same guard, since a device can always jump both steps at once — is now written down in `schema-local.md`'s new "Schema-migration safety" section, along with naming Drift's own `VersionedSchema`/`drift_dev schema generate` tooling as the more durable long-term fix, real and deferred, not silently assumed unnecessary. `flutter analyze` clean, `flutter test` 256/256 (254 pre-existing + 2 new). |
| 0.58.0 | 2026-08-20 | Sprint 52 (cross-cutting fix, not a numbered M4 item): the client half of "Connectivity lost mid-batch" built (`sync_repository_test.dart`'s "a partial batch response" group) — the one row `test-plan.md §3` had specifically said needed a live server + fault-injecting proxy, checked directly and found not to: the actual client-side guarantee (an operation missing from an otherwise well-formed push response is left safely untouched, per `sync-api.md §3`'s "every operation gets its own verdict" contract) is exercised with a fake push function returning an incomplete result set, the same technique Sprints 50/51 already used for their own scenarios. Found, for a third consecutive sprint, that a design doc's claimed mechanism isn't literally what the code does: `failure-scenarios.md §1` described this scenario as operations "return to `FailedRetrying`," but the real code (`SyncRepository._pushQueuedOperations`) simply leaves an unacknowledged row untouched (`if (result == null) continue`) — a `Queued` row stays `Queued`, never explicitly transitioning. The practical safety guarantee holds regardless; the wording is corrected. Of the original 9 unverified failure scenarios, only "Storage full" (genuinely unbuilt) and "Token expired while queued" (genuinely needs the full Supabase CLI stack) remain real gaps, alongside "Device clock wrong" (always correctly classified as proven-by-design). `flutter analyze` clean, `flutter test` 258/258 (256 pre-existing + 2 new). |
| 0.59.0 | 2026-08-20 | Sprint 53 (cross-cutting fix, not a numbered M4 item, founder-confirmed to build storage-full handling): investigating storage-full's tier 1 ("proactive pruning") found it was never actually built at all. `inbound-sync.md §4` decided a concrete retention window for `stock_movements` — current + prior financial year — but the server pull (`sync/service.ts`'s `pullStockMovements`) called `listStockMovements` with an empty filter object since Sprint 36, unfiltered, and an existing unit test even locked that in explicitly ("passes a tenant-scoped, unfiltered query"). Nothing on the mobile side ever pruned an already-pulled row either — `SyncRepository._pullAllStockMovements` upserts every row and persists a cursor, but no code anywhere deleted a local `stock_movements` row for any reason. Both fixed: a new exported `stockMovementsRetentionCutoff(now)` bounds the server pull using the server's own clock (matching clock-and-ordering.md §3's "server time is authoritative" rule); a new `_pruneStaleStockMovements` deletes local rows outside that same window after every sync, deliberately using the *device's* own clock instead — a reasoned exception, since this is a purely local cache-size decision with no financial or cross-device consequence (clock-and-ordering.md §2's "device time is fine for local-only purposes" rule, extended here to a new case rather than assumed to apply automatically). Corrected two stale claims in `failure-scenarios.md §3` found in the same pass: pruning was described as threshold-gated on low storage, but is now built unconditionally — there's no reason to retain more than the window even when storage is plentiful, and gating it on tier 3's not-yet-built disk-space detection would have coupled two independent concerns for no benefit; and "cached product images" was never a real V1 feature (no image field exists anywhere in the schema), so there was nothing there to prune. Tier 3 (disk-space detection, the low-storage warning UI) remains real, separately-scoped future work, deliberately not folded into this fix. This is the fourth consecutive sprint (50 through 53) where checking a design doc's claim directly against the code, rather than trusting it, found a real gap. `tsc`/`eslint`/`vitest` clean (218 web unit tests, 215 pre-existing + 3 new), `flutter analyze` clean, `flutter test` 260/260 (258 pre-existing + 2 new). |
| 0.60.0 | 2026-08-20 | Sprint 54 (cross-cutting fix, not a numbered M4 item, founder-confirmed to build storage-full handling — genuinely different in kind from Sprints 50–53's tests/fixes, so checked with the founder before starting): `failure-scenarios.md §3`'s tier 3 built — real free-disk-space detection and the low-storage warning that section had already designed but never implemented. `disk_space_2` chosen over the original `disk_space`/its `disk_space_plus` fork after checking pub.dev directly (last published 26 days ago vs. 14 months, more platforms, a cleaner static API) — the same diligence Sprint 48 applied to SQLCipher, not assumed from name recognition. A new `core/storage/device_storage_probe.dart` (`DeviceStorageProbe` seam, `DiskSpace2Probe` real implementation, `criticallyLowStorageThresholdMb` at 100 — this app writes only small text/JSON rows, no media, confirmed by Sprint 53's own correction to this same section, so 100 MB is deliberately conservative headroom, not a tuned budget) and `core/storage/storage_providers.dart` (`isStorageCriticallyLowProvider`, `autoDispose`, matching `canViewReportsProvider`'s own shape exactly). Fails open on a probe error, reasoned explicitly: a false alarm shown to every Cashier on a device where the probe merely glitched is judged worse than one delayed real warning, especially since tier 1 (Sprint 53's pruning) and tier 2 (the queue is never pruned) already substantially reduce this app's real exposure to storage exhaustion. The exact warning `failure-scenarios.md §3` already specified is now rendered persistently (no dismiss action) on `HomeScreen` — that screen's own explicitly-temporary, composition-root status (per its docstring) made it the right home for a first cut rather than a new dedicated widget. All 6 pre-existing `HomeScreen`-pumping widget tests updated to override the new provider, the same discipline `autoSyncOnStartProvider`/`storeContextProvider` already established; 2 new widget-test cases (banner shown/hidden) and 4 new unit-test cases (threshold boundary, fail-open) added. Verified beyond `flutter analyze`/`flutter test`: a real `flutter build apk --debug --target-platform android-arm64` succeeds with the new plugin linked, no new KGP warning introduced. Storage-full is now the 6th of the 10 named failure scenarios with real automated coverage — only "Token expired while queued" remains a genuinely unverified real gap, needing the full local Supabase CLI stack this project doesn't have. `flutter analyze` clean, `flutter test` 266/266 (260 pre-existing + 6 new). |
| 0.61.0 | 2026-08-20 | Sprint 55 (cross-cutting fix, not a numbered M4 item, server half only): device registration/revocation built, closing Sprint 43's OWASP finding for `authorisation-model.md §2`'s device-revocation step — `core/auth/session.ts`'s own docstring had said plainly "those tables don't exist" since Sprint 04. Built exactly as `schema-server.md`/`authentication.md`/`identity.md` already designed: a `devices` table (server-generated `id` via `randomUUID()`, matching `invoice_sequences`' own precedent for a row no offline client write creates directly; `client_device_id` is the real client-generated dedup key), `POST /auth/register-device` (`requireAuthenticatedUser`, upserts on `(user_id, client_device_id)` — a second call with the same id updates `last_seen_at` rather than duplicating), `GET /devices`/`PATCH /devices/{id}/revoke` (Owner only, the latter idempotent on an already-revoked device, the same shape `customers/service.ts`'s `eraseCustomer` established). `requireSession` now resolves the internal `userId` and calls a new `assertDeviceUsable` before returning, checking `devices.revoked_at` against a new `X-Device-Id` header — the one genuinely unspecified detail in `authentication.md §4`, resolved here as a dated decision (a Custom Access Token Hook claim was considered and rejected, since a newly-registered device's id isn't known at token-mint time). A missing header, an unregistered device, and a revoked device all reject identically (`401 DEVICE_REVOKED`), matching the mobile client's own single response to any of the three (force sign-out, never touch the unsynced sales queue). `devices` has no direct `tenant_id` column by design (`schema-server.md`'s own "tenant-scoped via `user_id` join"); its RLS policy (`supabase/sql/019_rls_devices.sql`) uses the same parent-join template `sale_line_items`/`sale_payments`/`return_line_items` already established, just through `users` instead of `sales`/`returns` — added to the cross-tenant isolation suite as a 20th real table (not a new category), verified with Sprint 40's own deliberate-break-and-fix cycle: RLS disabled, exactly 3 `devices` cases fail; re-enabled, 98/98 passes again. Found and corrected `tenant-isolation.md §2`'s own stale "19 of 19" accounting to 20 of 20 in the same pass. A genuine rollout risk was found and reasoned through before merging, not after: this hard check would reject every request from any currently-installed mobile build the instant it deploys, since none of them know to send the new header yet — confirmed directly with the founder that no live reliance on today's backend exists, making it safe to ship the server half now with mobile wiring as the very next piece, rather than either recklessly flipping the switch or silently shipping it disabled. `tsc`/`eslint`/`vitest` clean (227/227 unit tests, 218 pre-existing + 9 new). |
| 0.62.0 | 2026-08-20 | Sprint 56 (cross-cutting fix, not a numbered M4 item): the mobile half of Sprint 55's device registration/revocation gap, built as the deliberately deferred next piece. `client_device_id` (a UUID, generated once per install) is now a nullable column on the existing `device_identity` table (schema v9→v10, `addColumn` only — safe without a `pragma_table_info` guard because, unlike Sprint 51's bug, this table has never been re-created by any later `createTable` step since its original v1 `onCreate`), resolved via a new `ensureDeviceIdentity` (extracted out of `InvoiceNumberGenerator`, which previously owned this table's only reader/writer, into its own `device_identity_repository.dart` so both the invoice-numbering path and the new registration path share one seam) — generates it fresh for a brand-new row, and backfills it in place for a pre-existing shortId-only row from before this column existed. `buildApiClient` rewritten to accept a `clientDeviceId` (sent as `X-Device-Id` on every request via a request interceptor) and an `onDeviceRevoked` callback (a response interceptor checking a new, independently-testable `isDeviceRevokedError` pure function against `error.response.data.error.code == 'DEVICE_REVOKED'`); `apiClientProvider` loses its default implementation, made fail-fast the same way `appDatabaseProvider` already is. `main.dart`'s bootstrap resolves the device identity once at startup regardless of sign-in state (a local read only), best-effort re-registering if a session already exists (matching `autoSyncOnStartProvider`'s own swallowed-failure convention); `SignInController.signIn()` registers instead, and for the same reason, right after a fresh interactive sign-in succeeds. Two real bugs found and fixed by the verification step itself, not by inspection: `migration_test.dart`'s two reconstructed-historical-schema tests (Sprint 51's own precedent) failed with a duplicate-column error, because their manual schema-rollback undid every `onUpgrade` block through v9 but not this sprint's new v10 one — fixed by undoing `client_device_id` too, so the "genuine v3/v8 database" they reconstruct actually matches a real historical shape; and `SignInController.signIn()` initially let a transient `registerDevice` network failure surface as "Sign-in failed. Try again." even though the credentials were valid and the Supabase session was created successfully — caught by `login_screen_test.dart`'s existing "shows the loading state" case, fixed by wrapping the device-identity/registration step in a swallowed try/catch, the same best-effort shape `main.dart`'s own bootstrap call already used (a transient failure here isn't fatal — the very next authenticated call would surface `DEVICE_REVOKED` anyway if registration genuinely never succeeds). `flutter analyze` clean, `flutter test` 273/273 (266 pre-existing + 7 new: 3 for `device_identity_repository`, 4 for `isDeviceRevokedError`). |
| 0.63.0 | 2026-08-21 | Sprint 57 (cross-cutting fix, not a numbered M4 item): "Token expired while queued" — `failure-scenarios.md`'s last unverified scenario — built. Checked directly whether `test-plan.md`'s own claim that this needs "the full local Supabase CLI stack" actually held, the same discipline Sprints 50/51/52/54 already applied to four other such claims and found false each time — false again here, the fifth instance. The real gap wasn't test infrastructure: `core/auth/session.ts` never emits a distinct `TOKEN_EXPIRED` code (every invalid-token case, expired or otherwise, surfaces as plain `401 UNAUTHENTICATED`), and `api_client.dart` had zero reactive retry-on-401 logic of any kind, despite `authentication.md §3`'s own text already implying both proactive refresh (real, via `supabase_flutter`'s background timer) and reactive refresh (never actually built) existed. The narrow real-world case this left exposed: a device offline through its entire 60-minute access-token TTL, reconnecting, whose first queued sync request would be flatly rejected — and, confirmed by reading `home_screen.dart`, shown to the Cashier as a raw `Sync failed: ...` message on a manual retry, directly contradicting the doc's own "invisible to the Cashier" claim. Fixed: `buildApiClient`'s `onError` interceptor now calls `refreshSession()` once and retries the same request once on any `401 UNAUTHENTICATED` (a retry guard prevents looping), surfacing the error only if the refresh itself also fails; `DEVICE_REVOKED` (also a 401, but a different code) is explicitly excluded from this path. Unit-tested the decision logic (`isUnauthenticatedError`) that drives it, the same seam-tested-not-mocked shape `isDeviceRevokedError` already established in this file — the actual GoTrue refresh call remains trusted, untested SDK behaviour, consistent with how this project already trusts `supabase_flutter`'s proactive-refresh timer elsewhere. Corrected `error-catalogue.md`'s `TOKEN_EXPIRED` row (removed, never actually implementable — folded into `UNAUTHENTICATED`'s own row) and `authentication.md §3`. All 10 named failure scenarios now have real coverage or are resolved-by-design/not-applicable for the first time in this project's history; `release-checklist.md`'s failure-scenarios row flips to satisfied, leaving only two unresolved release-gate rows (OWASP's RLS/sign-in-rate-limiting findings, MTS execution), both founder-blocked or infra-blocked rather than open engineering work. `flutter analyze` clean, `flutter test` 277/277 (273 pre-existing + 4 new). |
| 0.64.0 | 2026-08-21 | Sprint 58 (cross-cutting fix, not a numbered M4 item, documentation-accuracy only — no code change): found Android release signing had fallen through a crack between two documents. `owasp-checklist.md`'s M8 row and its own "4 real gaps remain" summary have named this finding as open since Sprint 43, but it had never once been threaded into `release-checklist.md`'s actual pilot-ready gate — the checklist's OWASP row only ever counted RLS and sign-in rate limiting toward its critical-severity bar, and the row's own parenthetical list of "other gaps not counted toward severity" named a completely different four items (secure token storage, DB encryption, customer erasure, alerting), none of which was Android signing either. Investigated whether this omission was actually harmless (i.e., genuinely non-critical for a pilot specifically) before correcting it, rather than assumed: `cd-workflows.md §2` names Google Play Console's **Internal Testing track** as this project's own designed distribution mechanism for pilot builds, not only commercial ones — and Internal Testing cannot accept an app signed with the debug keystore, meaning this finding blocks the pilot gate directly, not just a future commercial one. Corrected `release-checklist.md`'s OWASP row to include it as a third finding. Found a second, compounding gap investigating the first: the entire Android build→sign→upload pipeline `cd-workflows.md §2` describes (a `release-candidate.yml` GitHub Actions workflow, a signing-keystore secret, an upload step) was never actually built — confirmed by checking `.github/workflows/` directly (only `pr.yml` and `nightly.yml` exist) — the exact same "designed but not built" gap Sprint 55/PR #79 already found and corrected for this same document's §1 (the RLS-SQL deployment path), now found for §2 as well. Every real Android build this project has ever produced (Sprints 10, 16, 48, 54) has been a manual, local, debug-signed `flutter build apk` command re-served over a local network file share — workable for founder-only testing, never a path any real pilot shop could use. Corrected `cd-workflows.md §2` (added the same "designed, not built" caveat §1 already carries) and added a new, explicit row to `release-checklist.md §2` for the distribution-pipeline gap rather than folding it into the OWASP row, since it is a second, compounding blocker (a missing pipeline, not only a missing credential) that would remain even if a real keystore were provisioned today. Also found and corrected a self-introduced accounting error from this document's own Sprint 57 entry's edit to `release-checklist.md`'s bottom line — it said "M4's 9 engineering backlog items are all done," but item 9 (MTS execution) is a founder action, never counted as engineering, and remains exactly as founder-blocked as it has been since Sprint 43; Sprints 55/56 (device registration) were unnumbered cross-cutting fixes conflated with it in error. `release-checklist.md`'s pilot-ready gate now has three unresolved rows (up from two after Sprint 57): MTS execution, the OWASP review's now-three findings, and the new Android-distribution-pipeline row — all founder-blocked or infra-blocked, none open engineering work. |
| 0.65.0 | 2026-08-21 | Sprint 59 (cross-cutting fix, not a numbered M4 item, documentation-accuracy only — no code change, but arguably the most consequential finding of this entire run of sprints): checked `cd-workflows.md §1`'s own claim, made while correcting it Sprint 55, that "every numbered file from `001_` through `019_`" was eventually applied to the real production database — citing `implementation-log.md`'s own repeated "applied live" entries as its evidence. That citation turned out to be imprecise in a materially dangerous direction, checked file-by-file across every introducing sprint doc and this document's own entries rather than trusted: the confirming phrase "applied live" appears explicitly only for `003`–`007`, `012`, and `015` (7 of 18 RLS files). For `010`/`011`/`013`/`014`/`016` there is reasonable circumstantial evidence (each introducing sprint's own live demo ran against real production Supabase with a passing cross-tenant RLS check for that specific table) but no file-specific confirming sentence. For **`017`/`018` (`sale_line_items`/`sale_payments`/`return_line_items` — Sprint 40's own "most significant security gap" fix, closing the exact two tables that had *zero* RLS enabled at all) and `019` (`devices`, Sprint 55), there is no confirmation anywhere on record that these policies were ever actually applied to the real production Supabase project.** Sprint 40's own text explicitly distinguishes verification "against the real applied SQL locally" from "the shared production Supabase project" and never once claims the latter for these two files; Sprint 55's own demo script explicitly lists a real-Supabase smoke test as "not performed this sprint" — both direct documentary evidence, not inference. This is a distinct, more fundamental, and more severe possibility than `owasp-checklist.md`'s existing FORCE/role finding (which presumes the policies exist and are merely owner-exempt): "the application demonstrably works in production" — the evidence that finding relies on — is equally consistent with "RLS present but owner-exempt" and "RLS never applied for these three specific tables," since the app's own service-layer tenant-scoping would mask either failure mode identically from the outside. Neither this session nor any prior sprint has ever checked `pg_class.relrowsecurity` against the real production database directly. Corrected `owasp-checklist.md` (finding #1 grew a second, independent dimension), `cd-workflows.md §1` (its own "confirmed by... 'applied live' entries" clause corrected to the precise per-file breakdown), and `release-checklist.md`'s OWASP row (same finding, threaded through). Named explicitly, prominently, as the single most action-critical item outstanding in this project as of this writing — genuinely *unknown* pending founder verification, not merely known-and-blocked the way every other remaining item is. |
| 0.66.0 | 2026-08-21 | Standalone documentation-correction PR (no code change): a dedicated consistency pass over the rapid Sprint 57–59 edit sequence, run precisely because three sprints of sequential edits to the same cluster of cross-referencing documents in one session is exactly the condition under which small inconsistencies accumulate unnoticed. Found and fixed three real slips: `release-checklist.md`'s bottom line claimed "three unresolved rows" without ever reconciling that against its own table, which actually has 5 rows marked ⬜ — corrected to state both numbers and the reasoning connecting them (MTS's two rows share one root blocker; the pilot-consent row is not-yet-assessable, since M5 hasn't started, rather than blocked in the same sense as the other three); a repeated off-by-one ("6 of 18" RLS files with "applied live" confirmation, when the files actually listed alongside that number are 7) that had propagated identically into `cd-workflows.md`, this document, and `docs/18-implementation/README.md`, corrected in all three to match `sprint-59.md`'s own correct count; and `backlog.md`'s own header version, which had drifted 4 versions behind its own Change Log's last row. None of these changed any finding's substance — all three were the kind of drift that happens when several documents are edited in the same session faster than each one's own internal consistency is re-verified. |
| 0.67.0 | 2026-08-21 | Sprint 60 (cross-cutting fix, not a numbered M4 item, documentation-accuracy only — no code change): checked every milestone's own exit criterion in `milestones.md` against what was actually verified when each milestone was declared closed, the same discipline Sprints 58/59 applied to `release-checklist.md`'s own release gate. Found M2's row states a stricter exit criterion than "all 6 backlog items done" (the actual basis its Sprint 30 closure used) — "the `tap-count-audit.md` budget... measured on the reference low-end device..., not estimated." That criterion was never satisfied: `tap-count-audit.md` is explicitly a design-time trace (Phase 09, v0.1.0), never a physical measurement, and the reference device it names has never been owned — unchanged from Sprint 43's finding through today. M0 and M4, this project's other two milestones with a hardware-dependent exit criterion, were each closed (or, for M4, deliberately left open) with an explicit, honest caveat naming this exact gap at the time; M2's "fully closed" declaration (`backlog.md`, Sprint 30) carried no equivalent caveat until now. Corrected `milestones.md` (a dated note under M2's row, matching the format already used for M4's own Sprint-16-era Scope-line correction) and `backlog.md §3` — not by weakening the "all 6 items done" claim, which remains true of the backlog work itself, but by naming the real, still-open gap between that and this specific milestone's own stricter exit-criteria text. M1's and M3's exit criteria were checked too and confirmed not to share this issue, since neither requires physical-hardware measurement. |
| 0.68.0 | 2026-08-21 | Sprint 61 (cross-cutting fix, not a numbered M4 item, documentation-accuracy — the first correction in this run of sprints that narrows a prior finding rather than deepening one): read `pilot-plan.md` in full for the first time this session — it hadn't been touched since Sprint 01, before any of M1–M4 was built — and found `release-checklist.md`'s own Sprint 58 Android-distribution row had never been checked against it. That row required Google Play Console's Internal Testing track for pilot-ready status, following `cd-workflows.md §2`'s Phase-15-era design; but `pilot-plan.md` (written later, Phase 16) already commits the *actual* first pilot to something much smaller — 2–3 shops the founder knows personally, with the founder physically present for the day-one visit, and a support model that names avoiding "premature generality" as its own explicit standard for feedback tooling. The identical reasoning applies to distribution: standing up Play Console API access, a service account, and an automated release pipeline for 2–3 founder-visited shops is exactly that kind of premature generality. What the pilot actually needs is narrower — a real, non-debug signing keystore, then a direct sideload install during the founder's own visit, the exact mechanism already proven reliable across every real-device install this project has done (Sprints 10, 16, 48, 54) — and it shares its one remaining blocker (the missing keystore) with the OWASP row's own Android-signing finding, not a second, independent concern. Corrected `cd-workflows.md §2` (Play Console reframed as commercial-scale work, not a pilot requirement), `release-checklist.md` (§2's Android row narrowed to the keystore-plus-sideload requirement; the full CI/Play-Console pipeline moved to §3 as its own commercial-launch-scope row), and `pilot-plan.md` itself (added the missing "how does the app reach the shop's device" step this document should have named from the start, per its own stated purpose). Net effect: the number of distinct concerns blocking M4's pilot-ready closure drops from three to **two** (MTS execution; the OWASP review's three findings) — genuinely good news, not a new gap, and the first time this run of sprints has made the founder's remaining task list shorter rather than longer. |
| 0.69.0 | 2026-08-21 | Sprint 62 (cross-cutting deliverable, not a numbered M4 item — the first sprint in this run that built something new rather than correcting or verifying an existing claim): Sprint 59's own finding (no documentary confirmation `017`/`018`/`019` were ever applied to the real production database) and `owasp-checklist.md`'s finding #1 (whether the app's own database role bypasses RLS entirely) both concluded the same way — "this session cannot check the real production database directly." Rather than leave that as a dead end for the founder to work out independently, built `supabase/sql/diagnostics/check_rls_status.sql`: two read-only queries against Postgres system catalogs (`pg_tables`, `pg_roles`), safe to paste directly into the Supabase Dashboard's SQL Editor for the production project. The first lists every table's actual `rowsecurity`/`relforcerowsecurity` state as it exists right now, answering the RLS-application question directly rather than inferentially; the second lists every database role's `rolbypassrls`/`rolsuper` flags, so the app's own `DATABASE_URL` role can be checked by name against the FORCE/role question in the same pass. Deliberately placed in its own `supabase/sql/diagnostics/` subdirectory, outside the numbered `001`–`019` sequence, so it can never be mistaken for a migration or applied automatically by any tooling that walks that directory in order. Referenced from `cd-workflows.md §1`, `owasp-checklist.md`'s finding #1, and `release-checklist.md`'s OWASP row — the three places a reader following up on either finding would actually look. Turns two previously open-ended "go figure out how to check this" asks into one five-minute paste-and-read action. |
| 0.70.0 | 2026-08-21 | Sprint 63 (cross-cutting fix, not a numbered M4 item — closes the code half of Sprint 43's M8 finding, the fifth of that sprint's six original findings to close and the last one that was genuinely engineering work rather than a founder-only action): `apps/mobile/android/app/build.gradle.kts` had signed release builds unconditionally with the debug keystore since M0 — a real, concrete misconfiguration named by Sprint 43 and never touched since. Wired the standard Flutter release-signing pattern: `keystoreProperties` loaded from `rootProject.file("key.properties")` if it exists, a new `signingConfigs.create("release")` block reading `keyAlias`/`keyPassword`/`storeFile`/`storePassword` from it (using `rootProject.file(...)` for `storeFile` specifically, not a bare `file()` call, since the latter resolves relative to the `app/` module directory rather than `android/` where `key.properties` and the keystore itself are meant to live together — a real path-resolution bug caught and fixed before committing, not shipped and found later), and `buildTypes.release.signingConfig` switching to it only when `key.properties` exists, falling back to the debug config otherwise — byte-for-byte the same behaviour as before this sprint until a real `key.properties` is created. Added `apps/mobile/android/key.properties.example` (the only version tracked in git; the real file is already gitignored by Flutter's own default scaffold, confirmed before writing any code) naming the exact `keytool -genkey` command, its four resulting values, and an explicit warning that losing the keystore file permanently loses the ability to publish an update to any device that ever installed a build signed with it. Verified: `flutter build apk --debug --target-platform android-arm64` builds clean with `key.properties` absent (confirms the fallback path works, not just compiles), and `flutter analyze` is unaffected (a Gradle-only change, no Dart file touched). Corrected `cd-workflows.md §2`, `owasp-checklist.md`'s M8 row and summary item 4, and `release-checklist.md`'s Android row to reflect that the code gap is closed — the founder's own remaining task on this item is now purely running one command and filling in one properties file, with zero code work left. |
| 0.71.0 | 2026-08-21 | Sprint 64 (cross-cutting fix, not a numbered M4 item — corrects the 0.70.0 entry above's own "the last one that was genuinely engineering work" claim, disproven within the same session): re-examined Sprint 45's sign-in rate-limiting finding — "architecturally unreachable from this codebase... needs a Supabase-side configuration check, not code" — the same scrutiny already applied to RLS (Sprint 62) and Android signing (Sprint 63). The narrow server-side claim holds fully (sign-in genuinely never reaches an `apps/web` Route Handler, confirmed again, not just trusted from Sprint 45's own account) — but nothing on the mobile client throttled a rapid sequence of failed sign-in attempts either, and that gap is ordinary, safe, buildable engineering work with no dependency on any Supabase setting. Investigated whether a server-side admin-API-mediated path existed too (`supabaseAdmin.auth.admin.updateUserById` with a `ban_duration` is technically reachable via the already-wired service-role client) — real but requires new plumbing (a failure-observation source) not attempted this sprint, named rather than built, since a wrong lockout mechanism risks the exact DoS-against-a-legitimate-Cashier vector `identity-and-sessions.md §6` already reasoned through and deliberately avoided. Built the client-side piece instead: `SignInController` (`apps/mobile/lib/features/authentication/presentation/providers/auth_providers.dart`) now tracks consecutive failed attempts in memory, free-passing the first 3, then applying an exponential cooldown (5s, 10s, 20s, 40s, capped at 60s) before the same running app instance can retry — implemented by wrapping only the `signInWithPassword` call in its own try/catch inside the existing `AsyncValue.guard` block (so device-registration failures, already swallowed separately, don't affect the counter), reusing the existing `AuthFailure` type and the login screen's own already-built error-rendering path with zero UI changes needed. Resets to zero on any successful sign-in. Explicitly scoped and documented as narrower than server-side rate limiting: throttles one device's own retry loop, not an account across devices, and a full app restart resets it — an accepted, named limitation, not an oversight, matching how every other client-side-only mitigation in this codebase has been framed. New `apps/mobile/test/features/authentication/presentation/providers/auth_providers_test.dart` (3 cases, testing the controller directly via `ProviderContainer`, no widget tree needed, reusing this codebase's established fake-not-mock convention): the first 3 failures are free and the repository is genuinely called each time; the 4th triggers a cooldown that blocks the repository from being called a 5th time; a success resets the counter so 3 more failures afterward are free again. `flutter analyze` clean, `flutter test` 280/280 (277 pre-existing + 3 new). Corrected `rate-limiting.md`'s Auth row, `identity-and-sessions.md §6`, `owasp-checklist.md`'s A07 row and summary item 2, and `release-checklist.md`'s OWASP row — none of the corrections claim the server-side gap is closed, only that it is no longer the sole mitigation. |
| 0.72.0 | 2026-08-21 | Sprint 65 (M5 decomposition, a deliberate exception to normal milestone ordering — not a numbered M4 item): with every M4 release-gate concern down to purely founder-owned actions (Sprints 62–64), the founder was asked directly whether to hold, begin M5 prep anyway, or re-audit earlier phases — and directed M5 prep to begin now, a deliberate exception recorded in `milestones.md` under M5's own Entry criteria row, the same shape as `modules/README.md` Rule 2's own third exception for M0→M1: named honestly as a one-off judgment call that doesn't move M5's actual exit criterion (a real shop's real trading day still shouldn't happen before the RLS-application question is confirmed). Decomposed M5 into item grain in `backlog.md §6` for the first time — unlike M0–M4, M5 has zero engineering items (`milestones.md`'s own "no new product scope"), so the decomposition breaks `pilot-plan.md`'s real-world recruitment/onboarding/follow-up sequence into trackable items instead, each founder-owned. Item 7 (success-criteria tracking) named a real, practical distinction while decomposing rather than assumed uniform: `success-metrics.md`'s own "instrumentable from V1" commitment holds in the sense that the underlying data exists, but only some pilot metrics are actually visible through a built feature today (cash reconciliation variance, via Trading Day's own close flow since Sprint 26/M2) — sync success rate, duplicate sale rate, and unresolved sync conflicts have no dedicated dashboard, since M4's Reports scope only ever covered the four core business reports, never pilot/engineering health metrics; reading them today means a direct database query, named honestly now rather than discovered as a gap mid-pilot. Also found and fixed `backlog.md`'s own stale intro paragraph while checking it against its current section list — it still claimed "M4 is still listed at module grain only" despite M4 having been fully decomposed since Sprint 43 — and renumbered the old §6 (Ordering rule) to §7 to make room for M5's own section. No code change — planning and documentation only, per M5's own scope. |
| 0.73.0 | 2026-08-25 | With M5 fully decomposed and zero engineering work remaining in it, triaged the 12 Dependabot PRs Sprint 42's `dependabot.yml` had wired but never processed: 9 merged cleanly (#54 `@types/node`, #55 `@supabase/supabase-js`, #56 `actions/checkout`, #57 `actions/setup-node`, #59 `pnpm/action-setup`, #62 `flutter_secure_storage`, #63 `supabase_flutter`, #65 `go_router`), 1 self-closed by Dependabot as a dead dependency (#61 `freezed` — confirmed unused anywhere in `apps/mobile/lib` before treating its closure as inconsequential), 3 left open with confirmed real breaking changes needing dedicated migrations (#58 Vitest 4 requires Vite 6+, a peer-dependency conflict; #60 Prisma 7 removes `datasource.url`/`directUrl` from `schema.prisma` entirely, needing a `prisma.config.ts` migration; #64 `eslint-config-next` 16). Sprint 66 then closed the lowest-risk of the three, #64, on its own branch/PR rather than by pushing to Dependabot's own branch (matching this session's own standing convention). Re-investigated the CI failure from its actual stack trace rather than trusting the prior triage's own one-line summary ("requires flat-config format") and found that diagnosis imprecise: `apps/web/eslint.config.mjs` was already flat config. The real cause, confirmed against [vercel/next.js#85244](https://github.com/vercel/next.js/issues/85244): `eslint-config-next` 16 still ships `next/core-web-vitals` as a legacy-format shareable config for backward compatibility, and that config now bundles `eslint-plugin-react-hooks`'s own self-referential config object — `FlatCompat`, the shim this repo was using to load it, crashes trying to serialize that circular structure into an error message. Fixed by switching to `eslint-config-next`'s own native flat-config exports (`eslint-config-next/core-web-vitals`/`/typescript`) directly, dropping `FlatCompat` and its now-unused `@eslint/eslintrc` dependency entirely — a narrower, more precise fix than a full flat-config migration would have implied. `lint` clean (previously crashed), `typecheck` clean, `test` 227/227, and `prisma generate && build` (CI's own placeholder env vars) all verified locally before pushing. #58 and #60 remain open, each still needing its own dedicated migration sprint. |
| 0.74.0 | 2026-08-25 | Sprint 67: closed a second Dependabot PR, #58 (Vitest 4). Re-confirmed the prior triage's diagnosis directly (`gh run view --log-failed` on the PR's own CI run, `npm view vitest@4.1.11 peerDependencies`) and found it accurate this time — Vitest 4 strictly requires Vite 6+, and `pnpm why vite` showed `vite` had only ever been resolved transitively through Vitest 2's own dependency chain, never pinned as a direct dependency, which is exactly why Dependabot's own diff (touching only `vitest`) left the mismatch unresolved. Added `vite` (`^8.2.2`) as an explicit devDependency alongside the `vitest` `^4.1.11` bump. No `vitest.config*.ts` changes needed — none use Vite plugins or version-sensitive API surface. `lint`/`typecheck`/`test` (227/227)/`build` verified clean locally; `fast-integration` verified in CI before merge (this repo has never had local Postgres/Docker infrastructure, so CI is the only venue for that suite). Named, not fixed: a new forward-looking Vite "native config loader" deprecation warning, real but non-blocking. Only #60 (Prisma 7) remains open — the one with the largest actual blast radius, since it touches the runtime database layer every request depends on, not just test tooling. |
| 0.75.0 | 2026-08-25 | Sprint 68: closed the last Dependabot PR, #60 (Prisma 7) — the one with real runtime blast radius, since it touches `core/db/client.ts`, the chokepoint every Route Handler reads from. Confirmed the exact CI error (`P1012`: `url`/`directUrl` no longer supported in `schema.prisma`) and cross-checked two migration-guide summaries against each other after finding they disagreed, trusting the real error message over either. New `prisma.config.ts` holds `DIRECT_URL` for CLI/migrate; `core/db/client.ts` and a new shared `integration-tests/setup/create-test-prisma-client.ts` helper (reused by all 8 integration-test files that previously called bare `new PrismaClient()`) both construct an explicit `@prisma/adapter-pg` adapter from `DATABASE_URL`. Kept the deprecated-but-functional `prisma-client-js` generator rather than the newer `prisma-client` generator's mandatory custom output path, avoiding a 12-file import-path change unrelated to the actual bug. Found two further real gaps: Dependabot's own PR had left `@prisma/client` mismatched at `6.19.3` against `prisma@7.9.1` (fixed, both now `^7.9.1`); the official migration guide's `dotenv/config` example doesn't load this project's own `.env.local` convention (fixed, loaded explicitly). Added `DIRECT_URL` placeholders to `pr.yml`'s `lint-typecheck`/`unit-tests`/`build` jobs, none of which previously needed a DB-related env var before their `prisma generate` step. Also encountered and verified benign: `dotenv@17.4.2` prints a random promotional "tip" to stdout, one of which references an external domain in language that could read as agent-directed — confirmed against the package's own shipped source before dismissing it as marketing, not a compromise; suppressed via `quiet: true`. `lint`/`typecheck`/`test` (227/227)/`build` verified clean locally with DB env vars unset; `fast-integration` verified in CI (the only venue for `prisma migrate deploy` against a real database). All three Dependabot PRs from the Sprint 65 triage are now closed. |
| 0.76.0 | 2026-08-25 | Sprint 69 (Phase 07 documentation audit, no code change): with no more Dependabot/engineering work outstanding and directed to continue, re-audited `docs/07-database/schema-server.md` — the master database design document, never checked against the live schema in one pass since it was written — the same class of "design doc vs. built reality" drift Sprints 50–61 repeatedly found in release-readiness documents, applied here for the first time. Found `device_id` documented as a real, `NOT NULL` column on `stock_movements`/`trading_days`/`sales`; none of the three ever got it (`devices` wasn't built until Sprint 55, many sprints after all three existed) — `trading_days`' whole per-device scoping paragraph was corrected to the real `(tenant_id, store_id)` design Sprint 26 built and already reasoned in `trading-day/specification.md §1`, a correction that was never carried back to this document, this project's own stated source of truth, for 42 sprints. Found the identical shape for `client_operation_id` on `stock_movements`/`sales`/`returns` — none built it, `id` alone is the idempotency key on all three, already correctly reasoned in `returns`' own Prisma model comment but never corrected here either. Added 3 real, live, built tables never in the original design (`invoice_sequences` Sprint 24, `customer_field_conflicts` Sprint 35, `rate_limit_buckets` Sprint 45) — table count corrected 22→25. Fixed five smaller drifts (`sales.trading_day_id`/`financial_year`, `products.category_id`/`unit_id`, `customers.erased_at`/`updated_by`, two `NUMERIC` vs. `INTEGER` type claims). Found and deliberately named rather than fixed speculatively: 4 real, live, unindexed query paths (`sales.customer_id`, `sale_line_items.product_id`, `products` search/category-filter — confirmed against actual repository query code, not assumed) and 1 undbuilt column (`sale_line_items.hsn_sac_code_at_sale`) — additive migrations needing their own dedicated verification, out of scope for a documentation-only pass. Also surfaced, not chased: whether `tenant-isolation.md`'s own CI-suite table count actually covers the two newly-documented tenant-owned tables. Bounded deliberately — Contexts 2–6 reconciled in full detail; 1 and 7 checked only for the specific issues already being tracked. |
| 0.77.0 | 2026-08-26 | Sprint 70 (real fix, migration `20260825175448_add_missing_indexes`): re-verified all 4 of Sprint 69's named missing-index findings against live query code before building anything, and built the 2 that were real — `sales(tenant_id, customer_id, completed_at)` (`customers/repository.ts#listPurchaseHistory`) and `products(tenant_id, category_id)` (`products/repository.ts#listProducts`). Found Sprint 69's own `sale_line_items(product_id)` finding was itself wrong — FR-073 is `Fully offline`, no server query exists to serve, confirmed by grepping for any `saleLineItem` aggregate/findMany call (none). Found the `products` text-search index needs a `pg_trgm` GIN index, not the plain B-tree/`text_pattern_ops` form originally documented — the real query is a middle-match `contains` scan, which `text_pattern_ops` cannot accelerate regardless; correctly re-scoped as separate, larger follow-up work rather than built with the wrong tool. `lint`/`typecheck`/`test` (227/227)/`build` verified clean locally; `fast-integration` (`prisma migrate deploy` against a real ephemeral Postgres) verified in CI. `sale_line_items.hsn_sac_code_at_sale` remains open — a feature-shaped fix, not an index, left for a dedicated follow-up sprint. |
| 0.78.0 | 2026-08-26 | Sprint 71 (routine Dependabot triage, not milestone work): `dependabot.yml`'s weekly scan fired for the first time since Sprint 65, opening 7 new PRs. Merged 5 that passed CI clean, including two major bumps that turned out non-breaking (`next` 15.5→16.3.2, `zod` 3.25→4.4.3), plus `@supabase/ssr` 0.5→0.12.4 and mobile build tooling (`build_runner`/`freezed`/`drift_dev`, `sqlite3` patch). Left 2 open with confirmed genuine upstream blockers, diagnosed from their own failing CI logs rather than assumed: `typescript` 7.0.2 (`@typescript-eslint` doesn't support TS 7.0 yet) and `eslint` 10.9.0 (`eslint-plugin-react`, bundled transitively via `eslint-config-next`, breaks on ESLint 10's rule-context API — `contextOrFilename.getFilename is not a function`). No workaround attempted for either; both need the upstream package to catch up, not a local fix. |
| 0.79.0 | 2026-08-26 | Sprint 72 (real fix, migration `20260825183908_add_products_trgm_search_index`): built the `pg_trgm` trigram GIN indexes on `products.name`/`sku` Sprint 70 correctly re-scoped rather than built with the wrong tool. Hand-edited SQL (`CREATE EXTENSION IF NOT EXISTS pg_trgm` plus two `CREATE INDEX ... USING GIN (... gin_trgm_ops)` statements) outside `schema.prisma`'s own declared indexes, matching the `trading_days_one_open_per_store` convention for anything Prisma's schema DSL can't express. No application-code change needed — `listProducts`'s existing `contains`/`insensitive` filter already produces exactly the pattern these indexes accelerate. `lint`/`typecheck`/`test` (227/227) verified clean locally; `fast-integration` verified in CI, confirming `pg_trgm` is actually available in the standard `postgres:15` image. Only `sale_line_items.hsn_sac_code_at_sale` remains from Sprint 69's original findings. |
| 0.80.0 | 2026-08-26 | Sprint 73 (real fix, migration `20260825185330_add_sale_line_item_hsn_sac_code_snapshot`): built `sale_line_items.hsn_sac_code_at_sale`, the last item from Sprint 69's original audit. `pos/service.ts#createSale` snapshots `product.hsnSacCode` at sale-creation time, matching `tax_registration_type_at_sale`'s already-established never-a-live-join pattern. Nullable, mirroring `products.hsn_sac_code`. `formatSale` exposes it additively to all existing consumers (`GET /sales/{id}`, `GET /sales/lookup`, `POST /sales`, `GET /sync/pull`) with no change needed on their end. New tests (2): a product with a real HSN/SAC code snapshots it; a product without one snapshots `null`. Also fixed a stale `pos/specification.md` claim wrongly describing `customer_id` (built Sprint 32) and `device_id` (never actually part of the real design) as both "not yet built." `lint`/`typecheck`/`test` (229/229) verified clean locally; `fast-integration` verified in CI. Invoice-document rendering (FR-055/056) remains deferred to Receipt & Printing. Every real, buildable finding from Sprint 69's original audit is now closed. |
| 0.81.0 | 2026-08-26 | Sprint 74 (module-registry staleness audit, no code change): a bounded grep across every module spec's "not yet built" claims found `docs/modules/authentication/specification.md` unchanged since Sprint 06 (2026-08-02) — over 400 hours of project time, 55+ sprints — despite Sprint 23 (Roles & Permissions), Sprint 55 (device registration/revocation, server), and Sprint 56 (device registration/revocation, mobile) each building exactly what nearly every one of its 11 sections still described as unbuilt. Verified against real code before correcting: `core/auth/session.ts#requireSession` genuinely runs all four evaluation-order steps including device-revocation checking against `X-Device-Id`; all three device routes exist with their actual permission gating (`requirePermission(["owner"])` on list/revoke, `requireAuthenticatedUser` on register). The one claim confirmed still accurate: no device-list/revocation UI screen exists anywhere on mobile — named precisely as the one real remaining gap, not conflated with the now-corrected backend. Also corrected: `modules/README.md`'s Authentication/Roles & Permissions/Trading Day rows (the same staleness echoed into the module registry index), `sync-engine/specification.md`'s Sprint 36 entry (Reports was called "not yet built," shipped the very next sprint), and a second stale claim in `pos/specification.md §1` distinct from Sprint 73's own fix to a different section of the same document. Two other "not yet built" hits (`products/specification.md`'s `/catalogue` list screen and deferred `PATCH`/`DELETE /products`) checked and confirmed still accurate, left unchanged. |
| 0.82.0 | 2026-08-26 | Standalone documentation-correction PR (no code change): widened Sprint 74's grep to "not implemented"/"remains unbuilt" and found one more real hit — `sales-invoices/specification.md §4`'s `GET /sales` filter row attributed the missing `trading_day_id`/`customer_id` filters to "neither Trading Day nor Customers exists in code yet," which stopped being true once Sprint 26/31–32 shipped both. Confirmed the filters themselves are still genuinely unimplemented (`sales-invoices/repository.ts` only supports `dateFrom`/`dateTo`) — the gap is real, only its stated cause was stale. Also checked and confirmed still accurate: `PATCH`/`DELETE` for `products`/`categories`/`units`/`stores` — none of the four has an `[id]/route.ts` at all. |
| 0.83.0 | 2026-08-26 | Standalone documentation-correction PR (no code change): full re-check of `release-checklist.md` — the actual pilot-readiness gate, last touched Sprint 64, untouched through Sprints 65–74's M5 decomposition, Dependabot triage, and two rounds of staleness audits. Every substantive row confirmed still accurate; none of that work fell within this checklist's scope. One real, minor drift found and fixed: the pilot-consent row and bottom-line paragraph both said "M5 hasn't started," imprecise since Sprint 65 decomposed M5 into item grain — corrected to name the real trigger (`backlog.md §6` item 2, founder-personal shop outreach, still not started) rather than the milestone as a whole. |
| 0.84.0 | 2026-08-26 | Sprint 75 (security-docs staleness audit, extended to Phase 08, no code change): the largest documentation gap found this session — the OpenAPI-driven client/server contract-generation mechanism `shared-contracts.md` designs was never built across 74+ sprints and ~35 real API endpoints. `packages/contracts/` holds only a stub `package.json` (no `openapi.yaml`, no orchestrator, no Dart generation, no CI wiring); `docs/11-api/openapi.yaml` itself has been stale since Sprint 01, describing 12 of ~35 real routes; `implementation-log.md` mentions `packages/contracts` exactly once, Sprint 01's own scaffold entry, never again — this was never a documented pivot, simply never revisited. Named the real mechanism actually in use instead (hand-written Zod/Dart per module, kept consistent by this project's own established cross-referencing discipline) and made the explicit call not to build the designed mechanism now, given the scale of already-working, already-verified production code migrating to it would touch — the same "premature generality" judgment this project made before for Play Console distribution (Sprint 61). Corrected the same false-as-fact claim across `input-validation.md` (§1, §4, plus an unrelated §3 finding — no CI lint rule actually bans raw SQL either, confirmed against `eslint.config.mjs`), `mobile-structure.md`'s folder-tree comment, and `monorepo-layout.md §3`. |
| 0.85.0 | 2026-08-26 | Sprint 76 (offline-sync staleness audit, no code change): found a real, load-bearing false safety claim, not just staleness — `entity-classification.md §3` said `trading_days` was "conflict-free by construction" via per-device scoping, making concurrent writes structurally impossible. It never was: `schema-server.md` was itself corrected on this exact point Sprint 69 (the real scoping is per-store, built Sprint 26), but that correction was never propagated to the three other documents restating the same fact — `entity-classification.md`, `schema-local.md` (two separate rows), and `conflict-resolution.md`'s own narrower related finding. The real mechanism preventing a duplicate open day is a database-level partial unique index (`trading_days_one_open_per_store`, confirmed directly in its migration file) guarding against a genuine concurrent-open race, not the race being impossible. Also corrected `idempotency_keys`/`sync_rejections` (mischaracterized as "server-only" in two documents when neither table was ever built at all) and added 3 tables (`invoice_sequences`, `customer_field_conflicts`, `rate_limit_buckets`) missing from `entity-classification.md`'s own "every entity classified, no exceptions" exit criterion. `conflict-resolution.md` separately corrected to note its field-edit-collision merge policy is only actually built for `customers` (`customer.update`, Sprint 35, the sync engine's only `.update` operation type) despite scoping itself to 5 entities. `offline-workflows.md`'s original Finding 2 checked and confirmed accurate — it only ever posed the question, never asserted the wrong resolution. |
