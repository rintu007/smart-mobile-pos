...
| 09 | Navigation | [09-navigation](09-navigation/) | 🔵 In review — 41 routes mapped, tap-count audit found and fixed 1 over-budget workflow |
...
| 17 | Sprint Planning | [17-sprints](17-sprints/) | 🔵 In review — 2-week solo-sized cadence; M0 fully decomposed (20 person-days); Sprint 01 through Sprint 06 all closed |
| 18 | Implementation | [18-implementation](18-implementation/) | 🟡 In progress — Sprint 01 done (Identity/Auth), Sprint 02 done (`POST /api/v1/onboarding`, Company & Store Setup), Sprint 03 done (Flutter SDK installed, `apps/mobile` scaffolded, local Drift database built), Sprint 04 done (`POST /api/v1/products`, a real `requireSession` bug found and fixed), Sprint 05 done (`POST /api/v1/sales` with server-side recompute, no new bug found), and Sprint 06 done (mobile `/auth/login` — the first real Flutter screen — built and verified live against Supabase Auth), all demoed live/for real. Sprint 02, 04, 05, and 06 each closed a real gap first before writing code. |
