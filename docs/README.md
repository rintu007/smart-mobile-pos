...
| 17 | Sprint Planning | [17-sprints](17-sprints/) | 🔵 In review — 2-week solo-sized cadence; M0 fully decomposed (18 person-days); Sprint 01 through Sprint 05 all closed |
| 18 | Implementation | [18-implementation](18-implementation/) | 🟡 In progress — Sprint 01 done (Identity/Auth), Sprint 02 done (`POST /api/v1/onboarding`, Company & Store Setup), Sprint 03 done (Flutter SDK installed, `apps/mobile` scaffolded, local Drift database built), Sprint 04 done (`POST /api/v1/products`, a real `requireSession` bug found and fixed), and Sprint 05 done (`POST /api/v1/sales` with server-side recompute, no new bug found), all demoed live/for real. Sprint 02, 04, and 05 each closed a real spec gap first before writing code. |
