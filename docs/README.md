...
| 17 | Sprint Planning | [17-sprints](17-sprints/) | 🔵 In review — 2-week solo-sized cadence; M0 fully decomposed (21 person-days, now past the 15–20 estimate); Sprint 01 through Sprint 09 all closed |
| 18 | Implementation | [18-implementation](18-implementation/) | 🟡 In progress — Sprint 01 done (Identity/Auth), Sprint 02 done (`POST /api/v1/onboarding`, Company & Store Setup), Sprint 03 done (Flutter SDK installed, `apps/mobile` scaffolded, local Drift database built), Sprint 04 done (`POST /api/v1/products`, a real `requireSession` bug found and fixed), Sprint 05 done (`POST /api/v1/sales` with server-side recompute, no new bug found), Sprint 06 done (mobile `/auth/login` — the first real Flutter screen — built and verified live against Supabase Auth), Sprint 07 done (mobile product creation, `/catalogue/add`, verified against a real on-disk file), Sprint 08 done (`GET /api/v1/stores` + mobile fetch-and-cache, verified live with a cross-tenant RLS proof), and Sprint 09 done (mobile till screen `/pos` — cart, cash-only sale completion, an atomic multi-row local write, and ADR-0008's local invoice-numbering half), all demoed live/for real. Sprint 02, 04, 05, 06, 07, 08, and 09 each closed a real gap first before writing code. |
