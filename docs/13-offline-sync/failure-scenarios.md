...
| **Token expired while queued** | **Built, Sprint 57.** The Sync Engine's push/pull attempt receives `401 UNAUTHENTICATED` ([error-catalogue.md](../11-api/error-catalogue.md) — corrected Sprint 57, no distinct `TOKEN_EXPIRED` code exists server-side), silently refreshes via the refresh token ([authentication.md §3](../11-api/authentication.md#3-refresh-and-rotation)), and retries the same request once — invisible to the Cashier if the refresh succeeds, surfaced only if the refresh token has also expired. Sales themselves are never blocked by this either way; sync is a background concern the till screen never waits on. |
...
| 0.7.0 | 2026-08-21 | Sprint 57 — "Token expired while queued" built: `buildApiClient`'s response interceptor now retries once after a silent `refreshSession()` call on any `401 UNAUTHENTICATED`. Found `TOKEN_EXPIRED` was never a real, implementable server-side code — corrected here and in `error-catalogue.md`/`authentication.md §3`. All 10 named failure scenarios now have real coverage or are resolved-by-design/not-applicable. |
