...
| `GET /api/v1/sales` | **Built this sprint.** Cursor-paginated on `(completed_at, id)`. Filters: `date_from`, `date_to` (both optional, ISO 8601). `trading_day_id`/`customer_id` from sales.md's original filter list are still **not implemented** (confirmed directly against `sales-invoices/repository.ts`, Sprint 74) — **correction:** this was never revisited once Trading Day (Sprint 26) and Customers (Sprint 31–32) actually shipped; the original reason ("neither exists in code yet") stopped being true at that point, but the filters themselves remain a real, still-open gap, just for a different reason now — nobody has come back to add them. Role-scoped per §2 (Cashier: own sales only; Manager/Owner: store-wide). Returns **summary fields only** — `id`, `status`, `provisional_invoice_number`, `canonical_invoice_number`, `financial_year`, `subtotal_minor_units`, `grand_total_minor_units`, `completed_at` — not nested `line_items`/`payments`, a deliberate design choice to keep a list response bounded regardless of how many line items each sale has; the `/{id}` detail endpoint is where the full embedded shape lives. |
...
| 0.2.1 | 2026-08-26 | Sprint 74 (module-registry staleness audit, continued): found and corrected §4's stale reasoning for why `GET /sales`'s `trading_day_id`/`customer_id` filters aren't implemented — the filters genuinely still aren't (confirmed against `sales-invoices/repository.ts`), but the stated cause ("neither Trading Day nor Customers exists in code yet") stopped being true once Sprint 26/31–32 shipped both; nobody revisited this endpoint afterward. The gap is real and unchanged; only the explanation was wrong. |
