...
| POS | V1 | 🟢 [spec](pos/specification.md) | 🔨 (`POST /api/v1/sales` live, cash-only/no-discount/no-tax, now with a same-transaction `sale` stock movement per line item (Sprint 11) and a same-transaction `sale.completed` audit-log entry (Sprint 12); mobile till screen (`/pos`) live — local write path built and tested, verified against a real Drift database, including a multi-row atomicity proof; the mobile write path and the server endpoint are now actually connected (Sprint 14's mobile sync trigger); permission enforcement applied Sprint 23 (any active role)) | Products, Stock Ledger, Customers *(named exception — see spec §1: M0's minimal slice doesn't wait on Stock Ledger/Customers, per the M0 exception below)* |
