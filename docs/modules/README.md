...
| POS | V1 | 🟢 [spec](pos/specification.md) | 🔨 (`POST /api/v1/sales` live, cash-only/no-tax, now with a same-transaction `sale` stock movement per line item (Sprint 11) and a same-transaction `sale.completed` audit-log entry (Sprint 12); mobile till screen (`/pos`) live — local write path built and tested, verified against a real Drift database, including a multi-row atomicity proof; the mobile write path and the server endpoint are now actually connected (Sprint 14's mobile sync trigger); permission enforcement applied Sprint 23 (any active role); optional `trading_day_id` linking Sprint 26; **per-line Discount live and live-verified Sprint 27** (17/17) — `discount_percent_basis_points`/`discount_amount_minor_units`, `DISCOUNT_REQUIRES_APPROVAL` above threshold, `subtotal_minor_units` corrected to its always-documented post-discount meaning; no mobile UI for discount yet) | Products, Stock Ledger, Customers *(named exception — see spec §1: M0's minimal slice doesn't wait on Stock Ledger/Customers, per the M0 exception below)* |
