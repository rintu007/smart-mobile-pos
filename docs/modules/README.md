...
| Products | V1 | 🟢 [spec](products/specification.md) | 🔨 (`POST /api/v1/products` live — Sprint 19 added optional `category_id`/`unit_id`/`sku`/`barcode`/`hsn_sac_code`, existence/uniqueness validated, live-verified including a regression proof that mobile's original `{id, name, price_minor_units}` shape still works; mobile local write path (`/catalogue/add`) live, verified against a real on-disk file (Sprint 07); a locally-created product now actually syncs to the server (Sprint 14); `GET`/`PATCH`/`DELETE`, *required* category/unit, and mobile's own category/unit picker UI all deferred) | Categories, Units |
