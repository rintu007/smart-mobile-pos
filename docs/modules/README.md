...
| Products | V1 | 🟢 [spec](products/specification.md) | 🔨 (`POST /api/v1/products` live — Sprint 19 added optional `category_id`/`unit_id`/`sku`/`barcode`/`hsn_sac_code`; mobile `/catalogue/add` now requires picking a category/unit (Sprint 20, UI-level requirement only), local write path verified against a real on-disk file (Sprint 07), a locally-created product actually syncs to the server (Sprint 14) now carrying real category/unit values; `GET`/`PATCH`/`DELETE`, *server-required* category/unit, and barcode/SKU search still deferred) | Categories, Units |
