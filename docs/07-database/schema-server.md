...
| `hsn_sac_code_at_sale` | `TEXT` | nullable — built Sprint 73, a snapshot of `products.hsn_sac_code` (Context 2) at sale-creation time, [RR-003](../02-business-requirements/regulatory-requirements.md). Found documented but never built during Sprint 69's audit; the GSTIN/invoice-document *rendering* this feeds (FR-055/056) remains deferred to Receipt & Printing, unchanged — this column stores the correct number per line, not the document. |
...
| 0.1.8 | 2026-08-26 | Sprint 73 (real fix, migration `20260825185330_add_sale_line_item_hsn_sac_code_snapshot`): built `sale_line_items.hsn_sac_code_at_sale`, the last item from Sprint 69's original findings — a snapshot of `products.hsn_sac_code` taken by `pos/service.ts#createSale` at sale-creation time, RR-003's per-line GST field. Nullable, mirroring `products.hsn_sac_code`'s own nullability. Invoice-document rendering (FR-055/056) remains deferred to Receipt & Printing, unchanged — this column stores the number, not the document. Every real, buildable finding from Sprint 69's audit is now closed. |
