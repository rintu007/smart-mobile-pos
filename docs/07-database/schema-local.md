...
| **Client-editable, single-writer** | `trading_days`, `customers`, `shop_settings`, `categories`, `units`, `products` | Editable offline by Manager/Owner ([endpoints/catalogue.md](../11-api/endpoints/catalogue.md)) — **correcting this document's earlier v0.1.0 classification**, which grouped catalogue data with identity data under "server-authoritative" on the mistaken assumption that [FR-019](../03-functional-requirements/functional-requirements.md) (which concerns role changes specifically) justified restricting catalogue edits too. It does not: a Manager setting up a new product line from the shop floor, offline, is a real and valuable V1 capability, distinct from a permission change. **Correction (2026-08-26):** `trading_days` was never actually scoped per-device — `schema-server.md` was itself corrected on this point Sprint 69. It's scoped per-*store*, and the conflict risk this row originally denied entirely is real: two devices at the same store can race to open the same day, closed by a real database-level partial unique index (`trading_days_one_open_per_store`), not by the collision being structurally impossible — see [entity-classification.md §3](../13-offline-sync/entity-classification.md) for the full corrected account. `customers`, `shop_settings`, `categories`, `units`, and `products` each need an actual conflict policy, resolved in [13-offline-sync/conflict-resolution.md](../13-offline-sync/conflict-resolution.md). |
...
| `trading_days` | **Correction (2026-08-26):** the server holds this per-*store*, not per-device (schema-server.md's own Sprint 69 correction) — a device pulls its store's trading days, which in practice equals "its own" only because V1's target shop is overwhelmingly single-device, not because of a device-level filter that doesn't exist. |
...
| `sync_rejections`, `idempotency_keys` | **Correction (2026-08-26):** neither table was ever actually built, on either side — this row's "server-only" framing implied a real server-side table a device just doesn't read, which overstates what exists. Both remain purely designed (`schema-server.md`'s own Context 7); `idempotency_keys`' real substitute (client-generated `id` alone as the dedup key) was found Sprint 41. |
...
| 0.1.7 | 2026-08-26 | Sprint 76 (offline-sync staleness audit): corrected three references to `trading_days` being scoped per-device — it never was (Sprint 26 built it per-store; `schema-server.md` corrected itself on this Sprint 69, never carried back here) — and one to `idempotency_keys`/`sync_rejections` being "server-only," when neither table was ever actually built at all. Same root-cause errors as `entity-classification.md §3`'s own correction in the same pass. |
