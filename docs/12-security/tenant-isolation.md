...
1. Four of the originally-listed 22 tables (`product_variants`, `batches`, `idempotency_keys`,
   `sync_rejections`) **do not exist in the built schema at all** — `product_variants`/`batches`
   are named V2+/V4 stubs in schema-server.md itself; `idempotency_keys`/`sync_rejections` were
   never built. `devices`, the fifth table missing at the time this correction was first written,
   is now built (Sprint 55) — see point 4 below.
...
3. `user_store_roles` was miscategorised as "scoped via a join to `users`, no own `tenant_id`" — it
   ...
   standard direct-column policy, the same as every other Category-1 table.
4. **Updated Sprint 55:** `devices` (`docs/modules/trading-day/specification.md §1`'s own forward
   reference — "revisit when Authentication's device-registration slice lands" — anticipated this
   correctly) is now built, closing the OWASP review's other flagged `authorisation-model.md §2`
   gap. It joins via `user_id` to `users`, the same parent-join shape as `sale_line_items`/
   `sale_payments`/`return_line_items`, just through a different parent table — no new category
   needed, added to the existing "Joined via parent" row below.

Per the schema as actually built (20 real tables, not 22), categorised:

| Category | Tables | Test shape |
| --- | --- | --- |
| Direct `tenant_id` column | 15: `stores`, `users`, `user_store_roles`, `audit_log`, `categories`, `units`, `products`, `customers`, `trading_days`, `sales`, `returns`, `shop_settings` (`tenant_id` is itself the primary key), `invoice_sequences`, `stock_movements`, `customer_field_conflicts` | [tenancy-model.md §5](../07-database/tenancy-model.md#5-the-proof-automated-cross-tenant-negative-test-suite)'s standard read/update/delete-by-ID attempt |
| Joined via parent (no own `tenant_id`) | `sale_line_items`, `sale_payments`, `return_line_items` (via `sales`/`returns`), `devices` (via `users`, Sprint 55) | Same attempt shape, via the parent-join policy in [tenancy-model.md §2](../07-database/tenancy-model.md#2-the-rls-policy-template) — **found and closed a real gap building this suite**: `sale_line_items`/`sale_payments`/`return_line_items` had **no RLS enabled at all** until Sprint 40 (`005_rls_sales.sql`/`015_rls_returns.sql`'s own comments had claimed none was needed, relying solely on the API never issuing an unscoped query — exactly the single point of failure RLS-as-defence-in-depth exists not to depend on). Closed via `supabase/sql/017_rls_sale_line_items_sale_payments.sql`/`018_rls_return_line_items.sql`, the parent-join template this document already specified; `devices`' own policy (`019_rls_devices.sql`) was built correctly the first time, verified by the same deliberate-break-and-fix cycle. |
...
**15 + 4 + 1 = 20 of 20 real tables accounted for.** (An earlier draft of this checklist omitted
`user_store_roles` and `devices` entirely, undercounting both the category list and the total — found
and corrected during Phase 18 pre-implementation review; the 2026-08-18 correction fixed the count
for the 19 tables that existed then, and this 2026-08-20 update adds `devices` as the 20th, now that
it's genuinely built.)
...
attempt against all 20 tables above, authenticated as a real Postgres role (`authenticated`, no
...
all 20 real tables; it does not yet cover this one boundary.
...
| 0.3.0 | 2026-08-20 | Sprint 55: `devices` built, closing the fifth of the originally-listed-but-missing tables §2 named. Added to the "Joined via parent" category (via `users`) and the suite's own table count, now 20 of 20. Verified with the same deliberate-break-and-fix cycle Sprint 40 established: RLS disabled, suite fails (3 cases), RLS re-enabled, suite passes (98/98 total). |
